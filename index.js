...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀:
// The reason why nestedFunction can access internal variable is because nestedFunction is still nested within the function that uses internal variable giving it access to nestedFunction.
...
function lowerCaseNames(arr) {
  let lowercase_names = arr.map((names) => {
...
function lowPopulationAnimals(arr) {
  const lowerPop = arr.filter((item) => item.population < 5);
  return lowerPop;
}
...
function USApop(arr) {
  const popTotal = arr.reduce(
    (total, states) => (total += states.population),
    0
  );
  return popTotal;
}
